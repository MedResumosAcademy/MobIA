...
- ✅ V1 — **CRM / Funil de negócios** (`/corretor/negocios`): pipeline por etapa (novo → contato → visita → proposta → fechamento; ganho/perdido), valor do negócio, timeline de atividades, **conversão de lead → negócio** com um clique, e métricas de funil (taxa de conversão, valor em aberto) no dashboard do gestor. Conversão calculada no `@imobia/core` (`resumoFunil`, 7 testes)
- ⏭️ Próximo (V1): contato do lead + atividades/tarefas; depois V2 (chatbot, comunidade, ranking, newsletter — ver [ESCOPO §9.1/V2](docs/ESCOPO.md))
